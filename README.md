...
- **Affluence Index, revenue/payback, agri water + NPA-risk, and shareable links (June 19, 2026, later)**:
  - **💰 Affluence Index (free composite proxy)** (`affluence.ts`) — a 0–100 area-level wealth proxy built **entirely from signals we already pull**, so it ships at ₹0: property **₹/sqft** (99acres, weight 0.5), Google Places **`priceLevel` mix** (already captured, was barely used; weight 0.3), and **VIIRS night-lights luminance** (already computed, was discarding the absolute value; weight 0.2). Missing signals drop out and weights renormalise (fails open). Surfaced as a **💰 Affluence Index** card in the market-snapshot strip with its component breakdown — **deliberately labelled a proxy, not a household-income figure** (it reliably separates Koramangala from a tier-3 outskirt; it does not claim "34% of households earn >₹10L"). The honest answer to GeoIQ's black-box income data.
  - **Best-effort grounded income context** (`aiIncomeContext` in `agent.ts`) — one grounded Gemini call per locality that fetches whatever income evidence exists on the open web and **bands it with explicit scope** (`district`/`city`/`locality`). In practice this returns district/city per-capita (Statista, govt) — **never invented neighbourhood income** — shown as a clearly-labelled "AI estimate · district-level" supplement to the deterministic index. Cached 14 days; fails open to the index.
  - **📈 Revenue & payback** (`computePayback`, `parseRupeeRange` in `agent.ts`; `VERTICAL_ECONOMICS` in `companies.ts`) — `/site-analysis` now returns a grounded **monthly ₹ revenue range** plus a **derived payback period** (months to recover setup capex), computed with pure arithmetic from the revenue midpoint, an editable per-vertical **operating margin + setup capex**, netting out the rent it already estimates. Shows monthly profit, payback, and the **visible assumptions** behind the number, with a confidence pill — framed as an order-of-magnitude model, not an oracle. When costs exceed revenue it honestly reads "not viable at these costs." Brings back the buyer's language (₹ + months) the score alone never spoke.
  - **🚜 Agricultural water & resources (the farmer-loan use case)** (`agriContext.ts`) — for **agricultural / vacant-land** collateral, an OSM Overpass read of **nearby water bodies** (river, canal, lake, reservoir, tank, stream, well, spring) with **distance to each**, a **water-access score (0–100)**, and **farmland evidence** (is this a real agricultural belt). This is the **inverse** of the flood check: `flood.ts` asks "is water *on* the parcel" (bad/flood); this asks "is water *near* the parcel" (good/irrigable). A canal 600 m away strengthens repayment capacity. **Verified against live OSM** (rural Tamil Nadu → river "Kamandala Naga" 627 m, water-access 87/100). Uses `out center` so polygon ways return coordinates. Fails open.
  - **🏦 Geographic NPA-risk band** (`computeNpaRisk` in `loanAnalysis.ts`) — the Collateral Check now composes a **LOW / MODERATE / ELEVATED** band + 0–100 score from signals it already has: flood exposure (JRC) + rainfall (ERA5) + % shuttered businesses + irrigation/water access. Fully **auditable** (every point carries a stated driver). Framed loudly as **"geographic contribution ONLY — NOT a credit score; combine with CIBIL, income and title"** — regulator-safe and in its lane.
  - **📤 Shareable snapshot links** (`/share`, `/share/:id`; `shared_results` collection) — after any Site-Finder analysis, a **Share this analysis** button saves the full result blob and copies a `/?r=<id>` link. Opening it re-renders the **exact same UI, map, heatmap, pins and cards** for anyone — a read-only snapshot the viewer can fork locally (tweak weights / re-run as their own). Public-read Firestore rule; writes server-side via the Admin SDK.
  - **OSM verified working** — confirmed the public Overpass endpoint is healthy; the long-standing "OSM returns 0" was **rural data sparsity** for POI tags (no one mapped ATMs in a village bbox), **not** an endpoint failure. Water features are mapped well, so the agri water-search is reliable.
- **Night-time lights demand/growth signal & weights-UX honesty (June 19, 2026)**:
  - **Night-time lights (VIIRS) — fresh economic-vitality signal** (`nightlights.ts`) — reads NASA GIBS **VIIRS Day/Night Band "Black Marble"** monthly composites as public WMTS PNG tiles (no auth, no key, decoded with `pngjs`, same shape as the JRC water tiles). Gives an **area-level vitality score (0–100)** for *"how lit & economically active is this area now"* plus a **3-year brightening/dimming trend** (this month vs. the same month 3 years ago, which removes seasonal/festival lighting bias). Unlike Census 2011 it is **monthly-fresh**, so it sees the new township / IT corridor / dark-store belt. Read **area-wide** (whole bbox, like WorldPop) because the native resolution is ~500 m — honest at catchment scale, not per-hex. The 3-yr trend **nudges the growth sub-score (capped ±10)** so a brightening area reads as rising and a dimming one gets pulled down, without ever overriding the analyst-grade AI narrative. Surfaced as a 🛰️ **Nightlight Vitality** card in the market-snapshot strip (green = brightening, red = dimming). Fails open. *Deliberately used as a vitality/growth signal, **not** an income signal — a market street and a floodlit stadium both glow.*
  - **Weights panel shows the *effective* %** — the Settings sliders are free-drag, and the engine normalises them (divides each by the running total) before scoring. Previously the panel showed each slider's raw position, so four sliders could read "140%" even though the math was correct — confusing for a tool we want BFSI buyers to trust. Each slider now displays its **normalised, applied share** (e.g. 36%, not 50%), and the totals line reads **"Effective weights · always sums to 100%"**. No scoring behaviour change — just an honest display of what's actually applied.
...
| **Economic vitality + growth trend** | **NASA GIBS VIIRS DNB "Black Marble"** monthly composites (WMTS PNG tiles) | **Free, no key** | Area-level night-time-lights vitality (0–100) + a 3-yr brightening/dimming trend, decoded from public tiles with `pngjs` (`nightlights.ts`). Monthly-fresh, so it sees new development Census 2011 can't. Read area-wide (~500 m native res). Trend nudges the growth sub-score (±10, capped). Vitality/growth signal — **not** income. Fails open. |
...
└── functions/                     ← Cloud Functions source
    ...
    └── src/
        ...
        ├── nightlights.ts         ← NASA VIIRS night-lights tiles → area vitality + 3-yr growth trend
        ├── affluence.ts           ← free Affluence Index (₹/sqft + Places priceLevel + VIIRS luminance)
        ...
        ├── agriContext.ts         ← OSM nearby water bodies + farmland → irrigation/water-access (farmer-loan use case)
...
| POST | `/share` | `{result}` (the full analysis blob) | Saves a shareable snapshot → `{id}`. Open `/?r=<id>` to re-render the exact UI. |
| GET | `/share/:id` | — | The saved snapshot blob for a share link. |
